...
<div class="page-inner mg-b-50">
	<div class="d-sm-flex align-items-center justify-content-between pd-b-20">
		...
		<div class="mg-t-15">
			...
			<button type="button" class="btn btn-sm pd-x-15 btn-white btn-uppercase" onclick="window.location='<?php echo admin_url($this->mod.'/edit/?id='.urlencode(encrypt($res['post_id'])));?>'"><i data-feather="edit" class="mr-2"></i><?php echo lang_line('button_edit');?></button>
		</div>
	</div>
		...
		<div class="card-body">
			<div class="row justify-content-center">
				<div class="col-md-9 align-self-center detailpost">
					...
					<h4 class="title"><?php echo $res['post_title'];?></h4>
					
					<div class="mg-b-20">
						<em class="text-muted meta">
							<div>
								<span><i class="cificon licon-user"></i> <?php echo $res['user_name'];?></span>
								<span class="mr-1 ml-1">/</span>
								<span  class="mr-1"><i class="cificon licon-calendar"></i> <?php echo ci_date($res['datepost'], 'l, d F Y');?></span>
								<i class="cificon licon-clock"></i> <?php echo ltrim(ci_date($res['timepost'], 'h:i A'),'0');?>
								<span class="mr-1 ml-1">/</span>
								<span><i class="cificon licon-folder-plus"></i> <?php echo $res['category_title'];?></span>
								<span class="mr-1 ml-1">/</span>
								<span><i class="cificon licon-eye"></i> <?php echo $res['hits'];?></span>
								<span class="mr-1 ml-1">/</span>
								<span><i class="cificon licon-message-square"></i> <?php echo $res['comment'];?></span>
							</div>
						</em>
					</div>
					<?php if (post_images($res['picture'])): ?>					
						<img src="<?php echo post_images($res['picture']);?>" class="pic" />
						<?php if ($res['image_caption']): ?>
						<p class="imgcaption"><?php echo $res['image_caption'];?></p>	
						<?php endif ?>
						<br>
					<?php endif ?>
