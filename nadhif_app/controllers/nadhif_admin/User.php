...
class User extends Backend_Controller {
			...
			if($this->input->is_ajax_request()) {
				if($this->input->post('act')=='delete') {
					...
					$this->json_output(array(
						'data'            => $data,
						'recordsFiltered' => $this->user_model->datatable('_all_user', 'count')
					));
				}
			} else {
	...
	public function add() {
		...
		if($this->role->i('write')) {
			...
			} else if($this->input->method() == 'post') { // when submit without ajax.
				show_404();
			} else {
	...
	public function edit() {
		...
		if($this->role->i('modify')) {
			...
			if(!empty($data_user)) {
				...
				} else if($this->input->method() == 'post') { // when submit form without ajax.
					show_404();
				} else { // default view.
