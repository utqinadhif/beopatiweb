	...
	private function list_group() {
		...
		if($this->role->i('read')) {
			if($this->input->is_ajax_request()) {
				...
				} else if($this->input->post('act') == 'dataTableGroups') {
					$data = array();
		...
		if($this->role->i('read') && $this->role->i('delete')) {
			...
			if($pk == 'root') {
				...
				if($this->input->is_ajax_request()) {
					...
					} else if($this->input->post('act')=='tableGroupRole') {
	...
	public function list_role() {
		...
		if($this->role->i('read')) {
			if($this->input->is_ajax_request()) {
				if($this->input->post('act') == 'delete' && $this->role->i('delete')) {
					return $this->_delete_role();
				} else if($this->input->post('act') == 'dataTableRoles') {
					$data = array();
					...
					$this->json_output(array(
						'data'            => $data,
						'recordsFiltered' => $this->permissions_model->datatable('_data_roles', 'count')
					));
				} else {
					$response = false;
					$this->json_output($response);
